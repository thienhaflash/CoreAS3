package vn.display {
	...
	/**
	 * StateView provide a stated container that only contains only one child at each state
	 * 
	 * @author thienhaflash
	 * @version 0.1.0
	 * @update 26.Feb.2012
	 ...
	 * 		var mainView : StateView = new StateView(mcMain); //by default defaultState is '' which mean contains no child
	 * 		
	...
	public class StateView {
		...
		/**
		 * StateView provide a stated container that only contains one child at each state
		 ...
		 */
		public function StateView(target: Sprite, defaultState = '', tweenFunc: Function = null, states: Object = null) {
			_holder	= target;
			...
			if (states) {//map states to DisplayObject
				...
				for (var s :String in states) {
					pdo 	= StateViewUtils.getChildByName(target, states[s]);
					pdo ? _map[s] = pdo : trace(this, 'invalid State declared <' + s + '>, child named <'+states[s]+'> not found on ', target);
				}
			} else {
				_map = StateViewUtils.getChildrenMap(_holder);
			}
			
			StateViewUtils.removeChildren(_holder);
			state			= defaultState;
	 ...
	 ********************/
		...
		private function _onTweenComplete():void {
			...
			StateViewUtils.removeMe(_lastContent);
