package vn.core.mouse
{
	 ...
	 **********************/
		...
		public static function kill(target: DisplayObject): void {
			delete dict[target];
		}
		
		public static function get(target: DisplayObject, autoNew: Boolean = true): Drag {
			if (!dict) dict = new Dictionary();
			var d : Drag = dict[target];
			if (!d) d = new Drag(target);
			return d;
		}
	}
